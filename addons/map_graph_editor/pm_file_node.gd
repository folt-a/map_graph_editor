...
func init(path):
	...
	if nde_resource.resource_type == nde_resource.TYPE_TEXTURE:
		_texture_activate()
	#scene has a script
...
func _texture_activate():
	...
	$VB/HSeparatorTexture.visible = true
	resizable = true
...
func resize(size):
	...
	$VB/TextureRect.rect_min_size = size
	$VB/TextureRect.rect_size = size
